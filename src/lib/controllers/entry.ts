...
export namespace EntryFlags {
    ...
    export function setDeleted(flags: number, deleted: boolean): number {
        return deleted ? flags | EntryFlags.DELETED : flags & ~EntryFlags.DELETED;
    }
...
export class Entry {
    ...
    public static async fromRaw(
        ...
        if (rawEntry.label) {
            const { err, val } = await Entry.addLabel(query, auth, entry, rawEntry.label);
            if (err) return Result.err(err);
            entry = val;
        }
    ...
    public static async create(
        query: QueryFunc,
        auth: Auth,
        json_: PickOptionalAndMutable<DecryptedRawEntry, 'id' | 'flags' | 'decrypted' | 'created'>
    ): Promise<Result<Entry>> {
    ...
    public static async getTitles(query: QueryFunc, auth: Auth): Promise<Result<Entry[]>> {
    ...
    public static async getStreaks(query: QueryFunc, auth: Auth): Promise<Result<Streaks>> {
    ...
    public static async near(
        ...
        return await Entry.fromRawMulti(
            ...
            SELECT id,
                   ...
                   agentData
            FROM entries
            WHERE ((flags & ${EntryFlags.DELETED}) = ${deleted ? EntryFlags.DELETED : 0} OR ${
                deleted === 'both'
            })
              AND user = ${auth.id}
