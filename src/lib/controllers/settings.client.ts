...
namespace SettingsUtils {
    ...
    export function fillWithDefaults(map: Record<string, Settings>): SettingsConfig {
        const newMap = { ...map };
        for (const [key, config] of Object.entries(SettingsUtils.config)) {
            if (!newMap[key]) {
...
export type SettingsKey = keyof typeof SettingsUtils.config;
export type SettingsConfig = {
    [key in SettingsKey]: Settings<(typeof SettingsUtils.config)[key]['defaultValue']>;
};
